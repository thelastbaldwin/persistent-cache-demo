...
It uses pokeapi for no other reasons than it is open and free. You can browse through a paginated list of pokemon and view extremely sparse details on the details page.
...
The cache stores the latest data while fresh data is being fetched. Interestingly enough, the pokeapi makes liberal use of cache-control headers, meaning in most cases that the browser itself is not actually hitting the api. This allows the user to _immediately_ see data on the page, see up to date data shortly thereafter, and also protects the upstream api from receiving too many requests.
